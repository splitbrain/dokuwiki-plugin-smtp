...
class GeneralTest extends DokuWikiTest
{
    ...
    public function testPluginConf(): void
    {
        ...
        $this->assertEquals(
            gettype($conf),
            gettype($meta),
            'Both ' . DOKU_PLUGIN . 'smtp/conf/default.php and ' . DOKU_PLUGIN . 'smtp/conf/metadata.php have to exist and contain the same keys.'
        );
        ...
        if ($conf !== null && $meta !== null) {
            foreach ($conf as $key => $value) {
                $this->assertArrayHasKey(
                    $key,
                    $meta,
                    'Key $meta[\'' . $key . '\'] missing in ' . DOKU_PLUGIN . 'smtp/conf/metadata.php'
                );
            }
            ...
            foreach ($meta as $key => $value) {
                $this->assertArrayHasKey(
                    $key,
                    $conf,
                    'Key $conf[\'' . $key . '\'] missing in ' . DOKU_PLUGIN . 'smtp/conf/default.php'
                );
            }
        }
    }
